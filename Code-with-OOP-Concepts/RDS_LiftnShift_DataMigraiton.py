...
def main():
    ...
    s3.create_bucket(veriables.bucket_name)

    # STEP 2 Upload DB backup file to s3 bucket created in previous step.
    s3.upload_file_bucket(veriables.backup_path, veriables.bucket_name, 'apr.bak')

    # STEP 3 Create security group to allow communication on port 1433 for SQL instance.
    security_group_id = ec2.create_security_group(veriables.rdsecuritygroup, veriables.vpc_id)['GroupId']

    # STEP 4 Create a service role for RDS service. This role will be used to allow access on S3 bucket created
    # in step 1.
    role_arn = iam.create_role(rolename=veriables.rdsvpcrole, policydocument=veriables.rdspolicydocument)['Role']['Arn']

    # STEP 5 create a policy to allow RDS service access S3 bucket.
    policy_arn = iam.create_policy(policyname=veriables.rds_s3bucket_access_policy, description='RDS bucket access policy',\
                                   policydocument=veriables.rds_s3bucket_access_policydocument)['Policy']['Arn']

    # STEP 6 attach role and policy created in step 4 and step 5, to enable RDS and S3 communication
    iam.attach_policy_to_role(policyARN=policy_arn, rolename=veriables.rdsvpcrole)

    # STEP 7 create configuration group for SQL instance to have S3 as backup source
    rds.create_and_configure_optiongroup(veriables.dbinstancename, role_arn)

    # STEP 8 create SQL instance
    rds.create_sql_instance(veriables.instance_name, security_group_id)
